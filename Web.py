...
# --- 4. GLOBAL STYLE INJECTION (FOOLPROOF VERSION) ---
css_template = """
...
# Replace tags manually to bypass f-string brace interpretation bugs
...
with st.sidebar:
    ...
    if source_type == "Camera/Phone Stream":
        camera_input = st.text_input(
            ...
            value="0", 
            help="Enter '0' for built-in camera, or an IP stream URL like 'http://192.168.1.50:8080/video'."
        )
...
if run_stream:
    ...
    if not cap.isOpened():
        st.error(f"❌ Connection Fail: Couldn't open source `{video_path}`")
        if isinstance(video_path, str) and video_path.startswith("http"):
            st.warning("""
            **Phone Link Check:**
            1. Ensure phone & computer are sharing the **same local Wi-Fi**.
            2. Match full stream tags (ex: finish string with `/video` or `/live`).
            3. Paste the URL into an empty computer browser window. If it fails there, the phone app stream is closed.
            """)
        else:
            st.warning("**Webcam Check:** Ensure no secondary software (Zoom, Teams) is holding your camera lens open.")
    else:
        prev_time = time.time()
        frame_counter = 0
